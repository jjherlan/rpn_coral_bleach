...
>>>>>>> 2fc49340faef11fdab3a93f189ea82ecdd631f15
...
>>>>>>> 2fc49340faef11fdab3a93f189ea82ecdd631f15
```{r}
poci_bl.glm <- glm(cbind(total_count, failures) ~ location * depth, 
                    family = binomial(link = "logit"), 
                    data = rpn_poci_bleach.main)
```

```{r}
anova(poci_bl.glm, test = "Chisq")
```
...
```{r}
poci_pb.glm <- glm(cbind(total_count, failures) ~ location * depth, 
                    family = binomial(link = "logit"), 
                    data = rpn_poci_pb.main)
```

```{r}
anova(poci_pb.glm, test = "Chisq")
```
...
```{r}
poci_p.glm <- glm(cbind(total_count, failures) ~ location * depth, 
                    family = binomial(link = "logit"), 
                    data = rpn_poci_pale.main)
```

```{r}
anova(poci_p.glm, test = "Chisq")
```
...
```{r}
poci_h.glm <- glm(cbind(total_count, failures) ~ location * depth, 
                    family = binomial(link = "logit"), 
                    data = rpn_poci_healthy.main)
```

```{r}
anova(poci_h.glm, test = "Chisq")
```
